package top.buukle.generator.utils;

import org.apache.commons.lang.math.NumberUtils;

/**
 * @Author elvin
 * @Date Created by elvin on 2018/9/30.
 * @Description :
 */
public class NumberUtil extends NumberUtils {

    /** 空值*/
    public static final Integer EMPTY = null;
    /** 0*/
    public static final Integer INTEGER_ZERO = 0;
    /** 1*/
    public static final Integer INTEGER_ONE = 1;
    /** 2*/
    public static final Integer INTEGER_TWO = 2;
    /** 3*/
    public static final Integer INTEGER_THREE = 3;
    /** 4*/
    public static final Integer INTEGER_FOUR = 4;
    /** 5*/
    public static final Integer INTEGER_FIVE = 5;
    /** 6*/
    public static final Integer INTEGER_SIX = 6;
    /** 7*/
    public static final Integer INTEGER_SEVEN = 7;
    /** 8*/
    public static final Integer INTEGER_EIGHT = 8;
    /** 9*/
    public static final Integer INTEGER_NINE = 9;
    /** 10*/
    public static final Integer INTEGER_TEN = 10;
    /** 100*/
    public static final Integer INTEGER_HUNDRED = 100;
    /** 1000*/
    public static final Integer INTEGER_THOUSAND = 1000;
    /** 1分钟秒数*/
    public static final Integer INTEGER_ONE_MINUTES_SECOND = 60;
    /** 5分钟秒数*/
    public static final Integer INTEGER_FIVE_MINUTES_SECOND = 300;
    /** 一天秒数*/
    public static final Integer INTEGER_ONE_DAY_SECOND = 86400;
    /** 一周秒数*/
    public static final Integer INTEGER_ONE_WEEK_SECOND = 604800;
    /** 一年秒数*/
    public static final Integer INTEGER_ONE_YEAR_SECOND = INTEGER_ONE_DAY_SECOND * 365;

    public static final Long LONG_THREE = 3l;
}
